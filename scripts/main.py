...
def preprocess_pcd_to_bev(pcd_file, grid_resolution, x_range, y_range, z_max, roi_bounds, ransac):
    ...
    # Visualization of BEV grid and points
    plt.figure(figsize=(10, 10))

    # Plot BEV grid
    plt.imshow(
        bev_values.T, cmap='gray', origin='lower', extent=(x_range[0], x_range[1], y_range[0], y_range[1])
    )
    plt.colorbar(label="Grayscale Value")
    plt.title("2.5D BEV Grid with Points")
    plt.xlabel("X (meters)")
    plt.ylabel("Y (meters)")
